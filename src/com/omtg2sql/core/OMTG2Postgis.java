...
/**
 * Main class for converting OMTG schema to native PostgreSQL/PostGIS SQL.
 * This module generates SQL using native PostGIS types and functions,
 * without depending on the ast-postgis extension.
 */
public class OMTG2Postgis {
	...
	public OMTG2Postgis(OMTGSchema omtgSchema, String xmlDocName) {
		...
		this.postgisGenerator = new PostgisGenerator(
			"./" + xmlDocName + "-ddl-structures.sql",
			"./" + xmlDocName + "-static-control.sql",
			...
			if (omtgRel.typeEquals("conventional-generalization")) {
				...
				if (((OMTGConventionalGeneralization) omtgRel).isTotal()) {
					if (((OMTGConventionalGeneralization) omtgRel).isDisjoint()) {
						postgisGenerator.mapConventionalGeneralizationTotalDisjoint(
							(OMTGConventionalGeneralization) omtgRel, superClass, subClasses);
					} else {
						postgisGenerator.mapConventionalGeneralizationTotalOverlap(
							(OMTGConventionalGeneralization) omtgRel, superClass, subClasses);
					}
				} else {
					if (((OMTGConventionalGeneralization) omtgRel).isDisjoint()) {
						postgisGenerator.mapConventionalGeneralizationPartialDisjoint(
							(OMTGConventionalGeneralization) omtgRel, superClass, subClasses);
					} else {
						postgisGenerator.mapConventionalGeneralizationPartialOverlap(
							(OMTGConventionalGeneralization) omtgRel, superClass, subClasses);
					}
				}
			}
				...
				if (((OMTGConceptualGeneralization) omtgRel).isDisjoint()) {
					postgisGenerator.mapConceptualGeneralizationDisjoint(
						(OMTGConceptualGeneralization) omtgRel, superClass, subClasses);
				} else {
					postgisGenerator.mapConceptualGeneralizationOverlap(
						(OMTGConceptualGeneralization) omtgRel, superClass, subClasses);
				}
			}
		}
